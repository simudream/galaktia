...
class daoTestCase(TestCase):
    ...
    def test_map_creation(self):
        ...
        stationary_dao = StationaryDAO(self.session_instance)
        ...
        for i,j in ((x,y) for x in [0,9] for y in range(1,10)):
            ...
            stationary_dao.add(stationary)
            expected_result.append(stationary)
        stationary_dao.session.flush()
        stationary_dao.session.commit()
        all_stationaries = stationary_dao.all()
    ...
    def test_character_creation(self):
        ...
        self.assertEqual(dao.move(walter, 2, 2), True)
            # Move Walter to (2, 2, 0) 
        dao.move(walter,1,2)
        self.assertEqual(dao.move(walter, 2, 2, collide_objects=True), False)
        self.assertEqual(walter.pos(), (1, 2, 0))
        self.assertEqual(dao.move(walter, 3, 3, 0, warp=True), True)
